...
// 	err := m.DB.QueryRowContext(ctx, existsQuery, roomID).Scan(&roomExists)
// 	if err != nil {
// 		return false, err
// 	}

// 	if !roomExists {
// 		return false, fmt.Errorf("room with ID %d does not exist", roomID)
// 	}

// 	// Step 2: Check for availability by searching for conflicts in room_restrictions
// 	var numRows int
// 	availabilityQuery := `
// 		select
// 			count(id)
...
// 	err = m.DB.QueryRowContext(ctx, availabilityQuery, roomID, start, end).Scan(&numRows)
// 	if err != nil {
// 		return false, err
// 	}

// 	// If no conflicts are found (numRows == 0), the room is available
// 	if numRows == 0 {
// 		return true, nil
// 	}
// 	return false, nil
